...
# variables/strings that need to be used here
open_msg = "Succesfully opened:"
title = "Notepad-"
#sends that you've opened the app to the server, so then the
#system knows that it can enable scripts
print(open_msg, title, "...Thanks for using!")
...
def cmdSave():     #file menu Save option
    fd = filedialog.asksaveasfile(mode = 'w', defaultextension = '')
    if fd!= None:
...
helpMenu.add_command(label='Credits', command = cmdAbout)
